...
// Number of detected bodies. defaults to kinect.h BODY_COUNT
#ifndef MY_BODY_COUNT
#define MY_BODY_COUNT BODY_COUNT
#endif
//#define MY_BODY_COUNT 1 TODO
...
BOOLEAN isSomeoneTracked = false;
...
/// Constructor
/// </summary>
CBodyBasics::CBodyBasics(void(*callback_function)(const char*)) :
    ...
    m_pBrushHandLasso(NULL)
{
	...
	myfunctions = new Action(callback_function);
}
...
void CBodyBasics::Update()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        IBody* ppBodies[MY_BODY_COUNT] = {0};
        ...
        if (SUCCEEDED(hr))
        {
            ProcessBody(nTime, MY_BODY_COUNT, ppBodies);
        }
                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
							...
							myfunctions->activationFunc(pBody, jointPoints, nTime);
                        }
                    }
                }
            }
			if (isSomeoneTracked != newTrackedState) {
				//Notify App if a person was tracked
				myfunctions->isTracked();
				isSomeoneTracked = newTrackedState;
			}
